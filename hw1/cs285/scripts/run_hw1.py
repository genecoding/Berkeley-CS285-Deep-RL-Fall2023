...
def run_training_loop(params):
    ...
    print([str(k)+'='+str(params[k]) for k in dir(params) if not callable(k) and not k.startswith('__')])
    print()
    ...
    for itr in range(params['n_iter']):
        ...
        log_metrics = ((itr+1) % params['scalar_log_freq'] == 0)

        print("\nCollecting data to be used for training...")
        if itr == 0:
            # BC training from expert data.
            # expert data: 2 trajectories, 1000 steps each
        ...
        for _ in range(params['num_agent_train_steps_per_iter']):
          ...
          num_data = len(replay_buffer)  # the total number of transitions in the buffer
          indices = random.sample(range(num_data), k=params['train_batch_size'])
        ...
        if log_metrics:
            ...
            for key, value in logs.items():
                ...
                # if params['n_iter'] == 1:  
                #     # if only run 1 iter, use time instead of n_iter to make graph easier to observe
                #     logger.log_scalar(value, key, logs["TimeSinceStart"])
                # else:
