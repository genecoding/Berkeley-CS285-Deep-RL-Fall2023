...
class ModelBasedAgent(nn.Module):
    ...
    def update(self, i: int, obs: np.ndarray, acs: np.ndarray, next_obs: np.ndarray):
        ...
        normalized_inputs = (torch.cat((obs, acs), dim=1) - self.obs_acs_mean) / (self.obs_acs_std + 1e-8)
        normalized_outputs = ((next_obs - obs) - self.obs_delta_mean) / (self.obs_delta_std + 1e-8)
        pred_outputs = self.dynamics_models[i](normalized_inputs)
        
        loss = self.loss_fn(pred_outputs, normalized_outputs)
    ...
    def update_statistics(self, obs: np.ndarray, acs: np.ndarray, next_obs: np.ndarray):
        ...
        self.obs_acs_mean = torch.cat((obs, acs), dim=1).mean(dim=0)  # (ob_dim+ac_dim)
        self.obs_acs_std = torch.cat((obs, acs), dim=1).std(dim=0)  # (ob_dim+ac_dim)
        self.obs_delta_mean = (next_obs - obs).mean(dim=0)  # (ob_dim)
        self.obs_delta_std = (next_obs - obs).std(dim=0)  # (ob_dim)
    ...
    def get_dynamics_predictions(
        ...
        normalized_inputs = (torch.cat((obs, acs), dim=1) - self.obs_acs_mean) / (self.obs_acs_std + 1e-8)
        pred_outputs = self.dynamics_models[i](normalized_inputs)
        unnormalized_pred_outputs = pred_outputs * self.obs_delta_std + self.obs_delta_mean
        pred_next_obs = obs + unnormalized_pred_outputs
    ...
    def evaluate_action_sequences(self, obs: np.ndarray, action_sequences: np.ndarray):
        ...
        for acs in np.transpose(action_sequences, (1, 0, 2)):
            ...
            next_obs = np.stack([self.get_dynamics_predictions(i, obs[i], acs) 
                                 for i in range(self.ensemble_size)], axis=0)
            ...
            rewards = np.stack([self.env.get_reward(next_obs[i], acs)[0] 
                                for i in range(self.ensemble_size)], axis=0)
    ...
    def get_action(self, obs: np.ndarray):
        ...
        if self.mpc_strategy == "random":
            ...
            for i in range(self.cem_num_iters):
                ...
                if i == 0:
                    # initialize, by formulas for mean & std of uniform distribution
                    elite_mean = (self.env.action_space.low + self.env.action_space.high) / 2
                    elite_std = (self.env.action_space.high - self.env.action_space.low) / (12**0.5)
                else:
                    # resample from the refitted distribution
                    action_sequences = np.random.normal(elite_mean,
                                                        elite_std,
                                                        size=(self.mpc_num_action_sequences, self.mpc_horizon, self.ac_dim))
                    
                # evaluate & pick elite action sequences
                rewards = self.evaluate_action_sequences(obs, action_sequences)
                assert rewards.shape == (self.mpc_num_action_sequences,)
                elite_act_seq_idxs = rewards.argsort()[-self.cem_num_elites:]  # argsort: acsending order
                elite_action_sequences = action_sequences[elite_act_seq_idxs]  # (num_elites, horizon, ac_dim)
                # soft update
                ...
                # elite_mean, elite_std: (horizon, ac_dim)
            return elite_mean[0]
        else:
