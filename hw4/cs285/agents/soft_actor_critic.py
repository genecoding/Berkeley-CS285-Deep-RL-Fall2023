...
class SoftActorCritic(nn.Module):
    ...
    def q_backup_strategy(self, next_qs: torch.Tensor) -> torch.Tensor:
        ...
        if self.target_critic_backup_type == "doubleq":
            ...
            # sample two Q-values for each sample
            critic_idx = np.array([np.random.choice(range(num_critic_networks), 2, replace=False) 
                                   for _ in range(batch_size)])
            critic_idx1, critic_idx2 = critic_idx[:, 0], critic_idx[:, 1]
            next_qs1 = next_qs[critic_idx1, range(batch_size)]
            next_qs2 = next_qs[critic_idx2, range(batch_size)]
            # take the minimum
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            if self.use_entropy_bonus and self.backup_entropy:
                ...
                next_action_entropy = self.entropy(next_action_distribution).mean()
        ...
        return {
            ...
            # "critic_entropy": next_action_entropy.item(),  # only for 3.1.2
        }
    ...
    def entropy(self, action_distribution: torch.distributions.Distribution):
        ...
        action = action_distribution.rsample()  # (batch_size, ac_dim)
    ...
    def actor_loss_reinforce(self, obs: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            obs = obs.expand(self.num_actor_samples, -1, -1)
            q_values = self.critic(obs, action)
