...
def collect_mbpo_rollout(
    ...
    for _ in range(rollout_len):
        ...
        ac = sac_agent.get_action(ob)  # ob: (ob_dim), ac: (ac_dim)
        next_ob = np.mean([mb_agent.get_dynamics_predictions(i, ob[None], ac[None]).squeeze(0) 
                           for i in range(mb_agent.ensemble_size)], axis=0)  # (ob_dim)                 
...
def run_training_loop(
    ...
    for itr in range(1, config["num_iters"]+1):
        print(f"\n\n********** Iteration {itr} ************")
        # collect data
        ...
        # on iteration 1, plot the full learning curve
        if itr == 1:
            plt.plot(all_losses)
            plt.title("Iteration 1: Dynamics Model Training Loss")
            plt.ylabel("Loss")
            plt.xlabel("Step")
            plt.savefig(os.path.join(logger._log_dir, "itr_1_loss_curve.png"))
...
def main():
    ...
    parser.add_argument("--dir_name", type=str, default="", help="put log dir under this dir")
    parser.add_argument("--exp_name", type=str, default="")

    args = parser.parse_args()
    print(args)  # show hyperparameters

    config = make_config(args.config_file, args.exp_name)
    logger = make_logger(config, args.dir_name)
