...
def run_training_loop(args):
    ...
    for itr in range(args.n_iter):
        ...
        # observation in trajs_dict: (# of trajectories, # of steps, ob_dim)
        # reward in trajs_dict: (# of trajectories, # of steps)
        ...
        if (itr+1) % args.scalar_log_freq == 0:
            # save eval metrics
            ...
            for key, value in logs.items():
                ...
                # use `Train_EnvstepsSoFar` as required in the homework
                logger.log_scalar(value, key, logs["Train_EnvstepsSoFar"])
            # print("Done logging...\n\n")

            # assemble print here, print out at the first iter and every `show_every` iters
            if itr == 0 or (itr+1) % show_every == 0:
                print(f"\n********** Iteration {itr+1} ************")
                print("\nCollecting data for eval...")
                for key, value in logs.items():
                    print("{} : {}".format(key, value))
...
def main():
    ...
    print(args)  # show all hyperparameters
    ...
    exp = ['', 'CartPole-v0', 'HalfCheetah-v4', 'LunarLander-v2', 'InvertedPendulum-v4', 'Humanoid-v4']
    part = 'exp'+str(exp.index(args.env_name)) if args.env_name in exp else ''
    logdir = (
        logdir_prefix
        + args.exp_name
        + "_"
