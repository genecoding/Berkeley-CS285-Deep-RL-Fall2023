...
class ReplayBuffer:
    ...
    def initialize_with_dataset(self, dataset):
        """
        Initialize the replay buffer with dataset.
        """
        assert self.size == 0, "Initialize with dataset only when the replay buffer is empty."
        dataset_size = len(dataset)
        assert dataset_size <= self.max_size, "Dataset is larger than replay buffer capacity."

        observation = dataset.observations[0]
        action = dataset.actions[0]
        reward = dataset.rewards[0]
        next_observation = dataset.next_observations[0]
        done = dataset.dones[0]
        ...
        # resume data from dataset
        self.observations[:dataset_size] = dataset.observations[:dataset_size]
        self.actions[:dataset_size] = dataset.actions[:dataset_size]
        self.rewards[:dataset_size] = dataset.rewards[:dataset_size]
        self.next_observations[:dataset_size] = dataset.next_observations[:dataset_size]
        self.dones[:dataset_size] = dataset.dones[:dataset_size]
