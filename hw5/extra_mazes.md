...
* I finetune `IQL` on these mazes and by results divide them into 3 groups, each applies a set of hyperparameters. I also run `DQN` for comparison.
...
* When mazes become more difficult, IQL outperforms DQN more significantly, which shows IQL can get a good initialization from offline training and keep improving the performance effectively during online training. 
...
* Exploration (RND) / Last 20000 steps during finetune training of DQN and IQL
  ...
  If learning is successful, a path from the start to the goal would emerge from trajectories of the agent.
